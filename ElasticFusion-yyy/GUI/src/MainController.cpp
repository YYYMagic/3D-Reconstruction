...
{
    while(!pangolin::ShouldQuit() && !(eFusion->getTick() == end && quiet))
    {
        if(!gui->pause->Get() || pangolin::Pushed(*gui->step))
        {
            ...
            if(true)
            {
                ...
                std::fstream outFile("~/proj/img/flg.txt",  std::ios::in );
                ...
                cv::Mat img_left_rgb = cv::imread("~/proj/img/left.png");
                cv::Mat img_left_D = cv::imread("~/proj/img/left_elas.png", cv::IMREAD_UNCHANGED);
                if(!img_left_D.data || !img_left_rgb.data) {
                    continue;
                }

                //cv::imshow("aaaa", img_left_D);
                //cv::imshow("aa", img_left_rgb);
                cv::waitKey(30);
                int64_t time1 = time(0);

                //float k = 701.89 * 120.0; //f*b
                const int data_size = img_left_rgb.rows * img_left_rgb.cols;
                unsigned short data_D[data_size];

                
                ushort *p = nullptr;
                for(int i=0;i<img_left_D.rows;i++)
                {
                    p=img_left_D.ptr<ushort>(i);//获取每行首地址
                    for(int j=0;j<img_left_D.cols;++j)
                    {
                        data_D[i * img_left_D.cols + j] = p[j];
                    }
                }
                

                eFusion->processFrame(img_left_rgb.data, data_D, time1, currentPose, weightMultiplier); //const unsigned short * depth
